...
### 2026-05-03 — AdIntakeBot: компактное лого для Telegram (~52px высоты)

- **Разведка:** Bot API `sendPhoto` не задаёт высоту в «строках» — клиент рисует превью из пикселей файла; уменьшение исходника — стандартный приём ([issue node-telegram-bot-api #615](https://github.com/yagop/node-telegram-bot-api/issues/615)).
- **`assets/vformate-logo-telegram.png`:** 149×52px, собирается из `vformate-logo.png` скриптом `npm run build:telegram-logo` (devDependency `sharp`).
- **`resolveAgencyLogoPath`:** по умолчанию отдаёт `vformate-logo-telegram.png`, если есть; иначе полный PNG; `AGENCY_LOGO_PATH` по-прежнему имеет приоритет.
