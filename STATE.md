...
### 2026-05-06 06:45 UTC — SMOKE TESTS & METRICS SNAPSHOT: Оптимизация latency развёрнута

**Проведена post-deployment проверка оптимизации latency (ШАГ 1-3):**

**ШАГ 1: API Server Status ✅**
- `/health` — 200 OK (0.26s)
- `/ready` — 200 OK, redis="ok" (0.37s)
- API Server: **RUNNING** на Railway
...
- ⚠️ `/api/tasks/:projectId` (GET) — 502 Bad Gateway (требуется проверка SUPABASE_ANON_KEY на Railway)
...
**Критические находки:**
- ⚠️ `/api/tasks` возвращает 502 — вероятно missing/invalid `SUPABASE_ANON_KEY` или `SUPABASE_SERVICE_KEY` на Railway
  - ACTION: Проверить Railway переменные окружения (Settings → Variables)
  - Требуемые: `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_KEY` (скопировать из Vercel env)
- 💡 Кэширование работает — `/api/metrics/summary` быстро вернул данные (0.82s с холодным кэшем)
...
**Следующие шаги (критические):**
1. **ОБЯЗАТЕЛЬНО:** Проверить/обновить `SUPABASE_ANON_KEY` и `SUPABASE_SERVICE_KEY` на Railway
   - Railway → Settings → Variables → Check values
   - Скопировать из Vercel env если отсутствуют
2. Перезагрузить Railway service (Settings → Restart)
3. Повторить smoke test /api/tasks (должен вернуть 200)
4. Мониторить метрики 10-15 минут (стабилизация после прогрева)
5. Если avg_latency не упадёт к 300-400ms за 15 мин → escalate для analysis (Supabase bottleneck?)
