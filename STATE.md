...
- **Почему было «огромно»:** клиент Telegram масштабирует фото по **ширине** пузыря; файл 149×52 давал экранную высоту ~`52×(ширина/149)` (~100px+) и размытие.
- **`vformate-logo-telegram.png`:** теперь **400×48** (холст + логотип до 36px по высоте), ориентир экранной высоты ~40px при ширине чата ~340px; пересборка `npm run build:telegram-logo`.
- **`/start`:** `link_preview_options.is_disabled` на фото и на тексте приветствия (на случай ссылок в подписи).
- **Справка:** Bot API не задаёт высоту превью в «строках» ([обсуждение](https://github.com/yagop/node-telegram-bot-api/issues/615)); **`resolveAgencyLogoPath`** по умолчанию — `vformate-logo-telegram.png`, иначе полный PNG; **`AGENCY_LOGO_PATH`** перебивает встроенные файлы.
