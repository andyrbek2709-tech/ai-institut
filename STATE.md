...
### 2026-05-05 — Railway Deployment: api-server
- Dockerfile переписан с multi-stage build (builder → production)
- Добавлены `src/middleware/metrics.ts`, `src/routes/metrics.ts`, `src/services/metrics.ts`, `src/routes/auto-rollback.ts`, `src/services/auto-rollback.ts`
- Railway проект `enghub-api-server` создаётся; Redis добавляется как plugin
- ENV в Railway: `REDIS_URL` (auto от Redis plugin), `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_KEY`
- Деплой через `railway up` из `services/api-server/`
