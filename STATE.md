...
- **`isAffirmative`:** без `\b` (в JS кириллица не считается границей слова → «да» не распознавалось): бот сбрасывал `pending_finalize` и снова показывал бриф (закольцованность).
