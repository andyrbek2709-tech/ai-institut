// ⚠️ DEPRECATED (2026-04-27): ЗАМЕНЕНО НА src/components/meeting/MeetingRoomPage.tsx
// Старая P2P+LiveKit-гибрид комната в духе «один проект = одна вечная комната».
// Оставлено как fallback на случай отката новой системы видеовстреч.
// Не подключается из App.tsx. Безопасно удалить после стабилизации новой версии.

import { useState, useEffect, useRef } from 'react';
import { createClient } from '@supabase/supabase-js';
import { Room, RoomEvent, Track, type TrackPublishOptions } from 'livekit-client';
...
const SCREEN_SHARE_PUBLISH: TrackPublishOptions = {
  ...
  simulcast: false,
  // Сбалансированный профиль качества/стабильности
  screenShareEncoding: { maxBitrate: 3_000_000, maxFramerate: 20 },
};
const USE_LEGACY_SCREEN_SIGNALING = false;
// Corporate mode: don't run legacy P2P audio mesh, to avoid dual-media-path conflicts.
const USE_LEGACY_AUDIO_SIGNALING = false;
...
export function ConferenceRoom({
  ...
  const mouseCtrlGrantedToRef = useRef<string | null>(null);
  ...
  const [forceRelay, setForceRelay] = useState(() => localStorage.getItem('enghub_force_relay') === '1');
  const [livekitConnState, setLivekitConnState] = useState<'idle' | 'connected' | 'reconnecting' | 'disconnected'>('idle');
  const [livekitDiag, setLivekitDiag] = useState<{ audioSubs: number; screenSubs: number }>({ audioSubs: 0, screenSubs: 0 });
  const livekitDiagRef = useRef(livekitDiag);
  const relayAutoAttemptedRef = useRef(false);
  ...
  const livekitLocalTrackRef = useRef<any>(null);
  const livekitMicToggleInFlightRef = useRef(false);
  const livekitAudioCleanupTimersRef = useRef<Map<string, ReturnType<typeof setTimeout>>>(new Map());
  /** Короткие отписки LiveKit (simulcast/adaptive) не должны мгновенно чистить экран — иначе «щёлчки» на чёрный */
  const livekitScreenUnsubTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  type RtcPublishMode = 'full_av' | 'audio_only' | 'receive_only' | 'text_only';
  const [rtcMode, setRtcMode] = useState<RtcPublishMode>('full_av');
  const rtcModeRef = useRef<RtcPublishMode>('full_av');
  useEffect(() => { rtcModeRef.current = rtcMode; }, [rtcMode]);
  const micPublishStatusRef = useRef<'unknown' | 'ok' | 'fail'>('unknown');
  const screenPublishStatusRef = useRef<'unknown' | 'ok' | 'fail'>('unknown');
  const lastDowngradeKeyRef = useRef<string | null>(null);
  const audioOutputSupportedRef = useRef<boolean>(
  ...
  const recomputeRtcModeFromCapabilities = () => {
    const mic = micPublishStatusRef.current;
    const scr = screenPublishStatusRef.current;
    if (mic === 'fail' && scr === 'fail') return 'text_only' as const;
    if (mic === 'fail') return 'receive_only' as const;
    if (scr === 'fail') return 'audio_only' as const;
    return 'full_av' as const;
  };

  useEffect(() => {
    localStorage.setItem('enghub_force_relay', forceRelay ? '1' : '0');
  }, [forceRelay]);

  useEffect(() => { livekitDiagRef.current = livekitDiag; }, [livekitDiag]);
  useEffect(() => { micWantedRef.current = micEnabled; }, [micEnabled]);
  useEffect(() => { screenWantedRef.current = screenSharing; }, [screenSharing]);

  const applyAudioOutputToElement = async (el: HTMLAudioElement, deviceId?: string) => {
    if (!audioOutputSupportedRef.current) return;
    const target = deviceId || selectedAudioOutputId || 'default';
    const setSink = (el as any).setSinkId;
    if (typeof setSink !== 'function') return;
    await setSink.call(el, target);
  };
  ...
  const refreshAudioOutputs = async () => {
    ...
    try {
      ...
      const hasSelected = outputs.some(d => d.deviceId === selectedAudioOutputId);
      if (!hasSelected) {
        setSelectedAudioOutputId(outputs[0].deviceId || 'default');
      }
    } catch (e) {
  ...
  const testAudioOutput = async () => {
    ...
    try {
      ...
      testEl.autoplay = true;
      testEl.muted = false;
      testEl.volume = 1;
      testEl.srcObject = dest.stream;
      await applyAudioOutputToElement(testEl);
      osc.start();
      await testEl.play();
      ...
      setAudioOutputError(e?.name || 'Не удалось воспроизвести тестовый звук');
    } finally {
  ...
  useEffect(() => { mouseCtrlGrantedToRef.current = mouseCtrlGrantedTo; }, [mouseCtrlGrantedTo]);
  ...
  useEffect(() => {
    ...
    const onDeviceChange = () => { refreshAudioOutputs().catch(() => {}); };
    navigator.mediaDevices?.addEventListener?.('devicechange', onDeviceChange);
    return () => {
      navigator.mediaDevices?.removeEventListener?.('devicechange', onDeviceChange);
    };
  }, [isInRoom]); // eslint-disable-line

  useEffect(() => {
    if (!audioOutputSupportedRef.current) return;
    audioElemsRef.current.forEach((el) => {
      applyAudioOutputToElement(el).catch((e) => {
        console.warn('[Audio] setSinkId failed', e);
      });
    });
  }, [selectedAudioOutputId]); // eslint-disable-line
  ...
  useEffect(() => {
    ...
    ch.on('broadcast', { event: 'offer' }, async ({ payload }: any) => {
      if (!USE_LEGACY_SCREEN_SIGNALING) return;
      if (payload?.to !== String(currentUser?.id)) return;
      // Экран идёт через LiveKit SFU — P2P offer для экрана не используем
    ...
    .on('broadcast', { event: 'mouse_move' }, ({ payload }: any) => {
      if (!screenSharingRef.current) return;
      const granted = mouseCtrlGrantedToRef.current;
      if (!granted || String(payload?.from) !== String(granted)) return;
      setRemoteCursor({ x: payload.x, y: payload.y });
    })
    // ── Audio WebRTC ──
    .on('broadcast', { event: 'audio_offer' }, async ({ payload }: any) => {
      if (!USE_LEGACY_AUDIO_SIGNALING) return;
      if (payload?.to !== String(currentUser?.id)) return;
    ...
    .subscribe((status: string) => {
      if (status === 'SUBSCRIBED') {
        ...
        // Announce arrival only if we intentionally run legacy P2P audio mesh.
        if (USE_LEGACY_AUDIO_SIGNALING) {
          setTimeout(() => {
  ...
  useEffect(() => {
    ...
    // Демонстрация экрана через LiveKit SFU (до N зрителей на одном потоке)
    if (livekitRoomRef.current) return;
  ...
  const connectLivekitAudio = async (overrideForceRelay?: boolean) => {
    if (livekitRoomRef.current) return livekitRoomRef.current;
    const projectId = String(project?.id || '');
    const userId = String(currentUser?.id || '');
    if (!projectId || !userId) throw new Error('Missing project/user for LiveKit');
    ...
    const tokenRes = await fetch('/api/livekit-token', {
      ...
      body: JSON.stringify({
        projectId,
        userId,
        userName: currentUser?.full_name || currentUser?.email || `user-${userId}`
      })
    });
    if (!tokenRes.ok) throw new Error(`LiveKit token failed: ${tokenRes.status}`);
    const tokenData = await tokenRes.json();
    const lkUrl = tokenData?.url || process.env.REACT_APP_LIVEKIT_URL;
    const lkToken = tokenData?.token;
    if (!lkUrl || !lkToken) throw new Error('LiveKit credentials missing');

    const relay = typeof overrideForceRelay === 'boolean' ? overrideForceRelay : forceRelay;
    const room = new Room(({
      // adaptiveStream паузит поток, если SDK считает video «не видимым» — даёт мерцание чёрным
      adaptiveStream: false,
      dynacast: false,
      rtcConfig: relay ? ({ iceTransportPolicy: 'relay' } as any) : undefined,
      publishDefaults: {
        simulcast: false,
        screenShareEncoding: { maxBitrate: 3_000_000, maxFramerate: 20 },
      },
    ...
    setLivekitConnState('idle');
    setLivekitDiag({ audioSubs: 0, screenSubs: 0 });

    room.on(RoomEvent.Connected, () => setLivekitConnState('connected'));
    room.on(RoomEvent.Reconnecting, () => setLivekitConnState('reconnecting'));
    room.on(RoomEvent.Reconnected, () => setLivekitConnState('connected'));
    room.on(RoomEvent.Disconnected, () => setLivekitConnState('disconnected'));
    ...
    room.on(RoomEvent.TrackSubscribed, (track, publication, participant) => {
      ...
      if (track.kind === Track.Kind.Audio) {
        setLivekitDiag((p) => ({ ...p, audioSubs: p.audioSubs + 1 }));
        const peerId = participant.identity || publication.trackSid;
        const pendingCleanup = livekitAudioCleanupTimersRef.current.get(peerId);
        if (pendingCleanup) {
          clearTimeout(pendingCleanup);
          livekitAudioCleanupTimersRef.current.delete(peerId);
        }
        const remoteTrack = (track as any).mediaStreamTrack;
      ...
      if (track.kind === Track.Kind.Video && publication.source === Track.Source.ScreenShare) {
        setLivekitDiag((p) => ({ ...p, screenSubs: p.screenSubs + 1 }));
        if (livekitScreenUnsubTimerRef.current) {
          clearTimeout(livekitScreenUnsubTimerRef.current);
          livekitScreenUnsubTimerRef.current = null;
        }
        const remoteTrack = (track as any).mediaStreamTrack;
        if (!remoteTrack) return;
        // Обычно держим один и тот же stream instance, чтобы не дёргать srcObject.
        // Но если текущий трек умер, переключаемся на новый.
        const stableStream = remoteStreamRef.current;
        if (!stableStream) {
          setRemoteStream(new MediaStream([remoteTrack]));
        } else {
          const currentTrack = stableStream.getVideoTracks()[0];
          const mustReplace =
            !currentTrack ||
            currentTrack.id !== remoteTrack.id ||
            currentTrack.readyState === 'ended' ||
            currentTrack.muted;
          if (mustReplace) {
            if (currentTrack && currentTrack.id !== remoteTrack.id) {
              stableStream.removeTrack(currentTrack);
            }
            if (!currentTrack || currentTrack.id !== remoteTrack.id) {
              stableStream.addTrack(remoteTrack);
            }
            setRemoteStream(stableStream);
          }
        }
        hasRemoteRef.current = true;
      }
    ...
    room.on(RoomEvent.TrackUnsubscribed, (track, publication, participant) => {
      ...
      if (track.kind === Track.Kind.Video && publication.source === Track.Source.ScreenShare) {
        // При переподписках LiveKit может давать краткие unsubscribe.
        // Чистим экран с небольшой задержкой и отменяем очистку, если быстро придёт новый subscribe.
        if (livekitScreenUnsubTimerRef.current) {
          clearTimeout(livekitScreenUnsubTimerRef.current);
        }
        livekitScreenUnsubTimerRef.current = setTimeout(() => {
          livekitScreenUnsubTimerRef.current = null;
          setRemoteStream(null);
          hasRemoteRef.current = false;
          requestedFromRef.current = null;
      ...
      livekitAudioCleanupTimersRef.current.set(peerId, setTimeout(() => {
        livekitAudioCleanupTimersRef.current.delete(peerId);
  ...
  const reconnectLivekit = async (nextForceRelay: boolean) => {
    if (livekitReconnectInFlightRef.current) return;
    livekitReconnectInFlightRef.current = true;
    try {
      // Force reconnect to apply rtcConfig changes.
      ...
      setForceRelay(nextForceRelay);
      await connectLivekitAudio(nextForceRelay);
    } finally {
      livekitReconnectInFlightRef.current = false;
    }
  };

  // Auto "corporate relay" retry:
  // If someone is speaking/sharing (presence says so) but we still subscribe 0 audio/screen tracks after ~12s,
  // try reconnecting with relay enabled (TURN/TLS).
  useEffect(() => {
    if (!isInRoom) return;
    if (forceRelay) return;
    if (relayAutoAttemptedRef.current) return;
    if (livekitConnState !== 'connected') return;
    relayAutoAttemptedRef.current = true;
    const myId = String(currentUser?.id ?? '');
    const t = setTimeout(() => {
      const { audioSubs, screenSubs } = livekitDiagRef.current;
      const others = conferenceParticipantsRef.current.filter((p: any) => p && String(p.id) !== myId);
      const otherMicOn = others.some((p: any) => !!p.micEnabled);
      const otherScreenOn = others.some((p: any) => !!p.screenSharing);
      const needAudio = otherMicOn && audioSubs === 0;
      const needScreen = otherScreenOn && screenSubs === 0;
      if (needAudio || needScreen) {
        void reconnectLivekit(true);
      }
    }, 12000);
    return () => clearTimeout(t);
  }, [isInRoom, forceRelay, livekitConnState, currentUser?.id]); // eslint-disable-line
  ...
  const joinRoom = () => {
    ...
    setIsInRoom(true);
    onJoin(false, false);
    connectLivekitAudio().catch((err) => {
      console.error('[LiveKit] connect failed', err);
      onSendMsg('Не удалось подключить голосовой сервер. Проверьте настройки LiveKit.', 'text');
    });
  ...
  const leaveRoom = async () => {
    micPublishStatusRef.current = 'unknown';
    screenPublishStatusRef.current = 'unknown';
    lastDowngradeKeyRef.current = null;
    setAudioOutputError(null);
    setRtcMode('full_av');
    // Stop mic
    ...
    audioSourcesRef.current.forEach(src => { try { src.disconnect(); } catch {} });
    audioSourcesRef.current.clear();
    livekitAudioCleanupTimersRef.current.forEach((timer) => clearTimeout(timer));
    livekitAudioCleanupTimersRef.current.clear();
    if (livekitLocalTrackRef.current) {
      try { livekitLocalTrackRef.current.stop(); } catch {}
    ...
    // Stop screen (если остался локальный поток вне LiveKit)
  ...
  const attachRemoteAudio = (peerId: string, stream: MediaStream) => {
    ...
    applyAudioOutputToElement(el).catch((e) => {
      console.warn('[Audio] setSinkId failed for peer', peerId, e);
      setAudioOutputError((e as any)?.name || 'Не удалось применить устройство вывода');
    }).finally(() => {
      el!.play().then(() => {
  ...
  const unlockAudio = () => {
    ...
    audioElemsRef.current.forEach((el, peerId) => {
      applyAudioOutputToElement(el).catch(console.warn).finally(() => {
  ...
  const toggleMic = async () => {
    if (!isInRoom) return;
    if (livekitMicToggleInFlightRef.current) return;
    livekitMicToggleInFlightRef.current = true;

    try {
      if (!micEnabled && micPublishStatusRef.current === 'fail') {
        onSendMsg('Микрофон недоступен в этой корпоративной сети (политики/запреты). Вы можете продолжать слушать.', 'text');
        return;
      }
      const room = await connectLivekitAudio();
      const nextEnabled = !micEnabled;
      await withTimeout(
        room.localParticipant.setMicrophoneEnabled(nextEnabled, {
      ...
      livekitLocalTrackRef.current = (pub as any)?.track || null;
      setMicEnabled(nextEnabled);
      if (nextEnabled) micPublishStatusRef.current = 'ok';
      if (!nextEnabled) {
        setIsTalking(false);
        isTalkingRef.current = false;
      }
      await onPresenceUpdate({ micEnabled: nextEnabled, screenSharing, isTalking: false });
      return;
    } catch (err) {
      console.error('[LiveKit] toggle mic failed', err);
      micPublishStatusRef.current = 'fail';
      setRtcMode(recomputeRtcModeFromCapabilities());
      const errName = (err as any)?.name || 'unknown';
      const downgradeKey = `mic:${errName}`;
      if (lastDowngradeKeyRef.current !== downgradeKey) {
        lastDowngradeKeyRef.current = downgradeKey;
        onSendMsg(
          errName === 'NotAllowedError' || errName === 'SecurityError'
            ? 'Микрофон заблокирован политиками браузера/организации. Вы можете продолжать слушать, но включить микрофон не получится в этой сети.'
            : 'Не удалось включить микрофон в этой сети. Переходим в режим прослушивания.',
          'text'
        );
      }
      return;
    } finally {
      livekitMicToggleInFlightRef.current = false;
    }
  ...
  const toggleScreenShare = async () => {
    ...
    try {
      const room = await connectLivekitAudio();
      if (!screenSharing) {
        if (screenPublishStatusRef.current === 'fail') {
          onSendMsg('Демонстрация экрана недоступна в этой корпоративной сети (запрет/политика).', 'text');
          return;
        }
        await withTimeout(
        ...
        const lt: any = pub?.track;
        const mst = lt?.mediaStreamTrack as MediaStreamTrack | undefined;
        if (!mst) {
          try { await room.localParticipant.setScreenShareEnabled(false); } catch { /* ignore */ }
          onSendMsg('Не удалось получить видеотрек экрана. Попробуйте ещё раз.', 'text');
          return;
        }
        ...
        screenPublishStatusRef.current = 'ok';
        setRtcMode(recomputeRtcModeFromCapabilities());
      ...
      console.error('[LiveKit] screen share failed', e);
      screenPublishStatusRef.current = 'fail';
      setRtcMode(recomputeRtcModeFromCapabilities());
      const errName = (e as any)?.name || 'unknown';
      const downgradeKey = `screen:${errName}`;
      if (lastDowngradeKeyRef.current !== downgradeKey) {
        lastDowngradeKeyRef.current = downgradeKey;
        onSendMsg(
          errName === 'NotAllowedError' || errName === 'SecurityError'
            ? 'Демонстрация экрана заблокирована политиками браузера/организации. Вы сможете слушать, но экран сейчас недоступен.'
            : 'Не удалось запустить демонстрацию экрана в этой сети. Переходим в режим без экрана.',
          'text'
        );
      }
    }
  ...
  const handleVideoMouseMove = (e: React.MouseEvent<HTMLVideoElement>) => {
    ...
    const now = Date.now();
    if (now - lastMouseSendRef.current < 16) return; // ~60fps
  ...
  return (
    ...
    >
      ...
      {audioOutputError && (
        <div style={{
          position: 'absolute', top: audioBlocked ? 52 : 8, left: '50%', transform: 'translateX(-50%)',
          zIndex: 9998, background: '#F59E0B', color: '#111827',
          padding: '7px 14px', borderRadius: 16,
          fontSize: 12, fontWeight: 700, boxShadow: '0 4px 10px rgba(245,158,11,0.35)',
          display: 'flex', alignItems: 'center', gap: 8
        }}>
          ⚠️ Проблема вывода звука: {audioOutputError}
        </div>
      )}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          {isInRoom && (
            <div style={{
              ...
              background: livekitConnState === 'connected' ? "#10B98120" : livekitConnState === 'reconnecting' ? "#F59E0B20" : "#EF444420",
              border: `1px solid ${livekitConnState === 'connected' ? "#10B98150" : livekitConnState === 'reconnecting' ? "#F59E0B50" : "#EF444450"}`,
              fontSize: 12,
              color: livekitConnState === 'connected' ? "#10B981" : livekitConnState === 'reconnecting' ? "#F59E0B" : "#EF4444",
              fontWeight: 700
            }}>
              LK: {livekitConnState === 'connected' ? 'OK' : livekitConnState === 'reconnecting' ? 'reconnect' : livekitConnState}
              <span style={{ opacity: 0.8, fontWeight: 800 }}>A:{livekitDiag.audioSubs}</span>
              <span style={{ opacity: 0.8, fontWeight: 800 }}>S:{livekitDiag.screenSubs}</span>
              {forceRelay && <span style={{ opacity: 0.9, fontWeight: 900 }}>RELAY</span>}
            </div>
          )}
          ...
          {isInRoom && (
            <>
              <label style={{ display: "flex", alignItems: "center", gap: 6, padding: "0 8px", height: 34, borderRadius: 10, background: C.surface2, border: `1px solid ${C.border}`, color: C.textMuted, fontSize: 11, fontWeight: 800, cursor: "pointer" }} title="Корп-режим: принудительно через TURN/relay (обычно лучше под прокси)">
                <input
                  type="checkbox"
                  checked={forceRelay}
                  onChange={async (e) => {
                    ...
                    setForceRelay(v);
                    relayAutoAttemptedRef.current = false;
                    // reconnect LiveKit to apply rtcConfig
                    try { livekitRoomRef.current?.disconnect(); } catch {}
                    livekitRoomRef.current = null;
                    setLivekitConnState('idle');
                    void connectLivekitAudio(v).catch(() => {});
                  }}
                />
                Корп
              </label>
              {rtcMode !== 'full_av' && (
                <div style={{
                  ...
                  ⚠️ Режим медиа: {rtcMode === 'audio_only'
                    ? 'только аудио'
                    : rtcMode === 'receive_only'
                      ? 'прослушивание'
                      : 'только текст'}
                </div>
              )}
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                {audioOutputSupportedRef.current ? (
                  <>
                    <select
                    ...
                    >
                      {!audioOutputs.length && <option value="default">Системный вывод</option>}
                      {audioOutputs.map((d) => (
                        <option key={d.deviceId || 'default'} value={d.deviceId || 'default'}>
                          {d.label || 'Аудиоустройство'}
                        </option>
                      ))}
                    </select>
                    <button
                      ...
                      style={{
                        ...
                        cursor: isTestingAudioOutput ? "not-allowed" : "pointer",
                        background: "#3B82F620", color: "#3B82F6", fontSize: 12, fontWeight: 700
                      }}
                    >
                      {isTestingAudioOutput ? "..." : "🔊 Тест"}
                    </button>
                  </>
                ) : (
                  <div style={{
              ...
              {screenSharing && (
                <button onClick={toggleScreenShare} title="Остановить демонстрацию" style={{
          ...
          {!screenSharing && !!remoteStream && (
            <div style={{ position: "absolute", inset: 0 }}>
              <video
                ...
                style={{ width: "100%", height: "100%", objectFit: "contain", display: "block", background: "#0a0a0a", cursor: mouseCtrl === 'granted' ? 'crosshair' : 'default' }}
              />
            </div>
          )}
