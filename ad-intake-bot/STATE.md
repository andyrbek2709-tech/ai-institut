...
- Файл **`BOT_REFERENCE.md`**: структура проекта, как работает процесс, все настройки (env) и обработчики.

## 2026-05-03 — README обзор для онбординга

- Расширен **`README.md`** в этом каталоге: репозиторий, реализованный функционал, деплой, стек, ссылки на **`STATE.md`** и **`.env.example`**.

## 2026-05-03 — лого для чата (trim + динамический холст)

- `vformate-logo-telegram.png` — trim, целевая экранная высота ~28px; `npm run build:telegram-logo`.

## 2026-05-03 — /start: лого + kk/ru, без «Шаблоны» в приветствии

- После фото: kk и ru с «компания в/v … ${AGENCY_NAME}»; строка «Шаблоны: /templates» из приветствия убрана (команда `/templates` в /help остаётся).
