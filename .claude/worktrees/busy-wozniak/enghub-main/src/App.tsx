...
export default function App() {
  ...
  useEffect(() => { if (!showUserMenu) return; const close = () => setShowUserMenu(false); document.addEventListener('mousedown', close); return () => document.removeEventListener('mousedown', close); }, [showUserMenu]);
  useEffect(() => {
    if (showTaskDetail && selectedTask && token) {
      const pid = activeProject?.id || selectedTask.project_id;
      if (pid) loadMessages(pid, selectedTask.id);
    }
  ...
  const sendMsg = async (taskId?: number, type: string = "text", customText?: string) => {
    ...
    const projectId = activeProject?.id || selectedTask?.project_id;
    if (!finalTxt.trim() || !projectId || !currentUserData?.id) return false;
    ...
    try {
      ...
      await loadMessages(projectId, taskId);
      return true;
    } catch (err: any) {
      ...
      <div className="main-area">
        ...
        <div className="topbar">
          ...
          <div className="topbar-right">
            ...
            {currentUserData && (
              <div className="topbar-user" style={{ position: 'relative', cursor: 'pointer' }} onClick={() => setShowUserMenu(v => !v)}>
                <AvatarComp user={currentUserData} size={34} C={C} />
                ...
                {showUserMenu && (
                  <div style={{ position: 'absolute', top: '110%', right: 0, zIndex: 999, background: C.card, border: `1px solid ${C.border}`, borderRadius: 10, boxShadow: '0 4px 20px rgba(0,0,0,0.15)', minWidth: 160, padding: '6px 0' }} onClick={e => e.stopPropagation()}>
                    <button onClick={handleLogout} style={{ width: '100%', padding: '10px 16px', background: 'none', border: 'none', color: '#e74c3c', fontWeight: 600, fontSize: 14, cursor: 'pointer', textAlign: 'left' }}>
                      🚪 Выйти
                    </button>
                  </div>
                )}
        ...
        <div className="content">
          ...
          {screen === "dashboard" && (
            <div className="screen-fade">
              ...
              {(() => {
                ...
                const overdueProjects = projects.filter(p => { const dl = parseDeadline(p.deadline); return dl && dl < now && p.status !== 'done'; }).length;
                return (
                  <div className="stats-row">
                    {((isGip || isAdmin) ? [
                      { label: "Проектов", value: projects.length, color: C.accent, onClick: () => setScreen("projects") },
                      { label: "Активных задач", value: allTasks.filter(t => t.status !== "done").length, color: C.blue, onClick: () => setScreen("tasks") },
                      { label: "На проверке ГИПа", value: allTasks.filter(t => t.status === "review_gip").length, color: C.purple, onClick: () => setScreen("tasks") },
                      { label: "Просроченных проектов", value: overdueProjects, color: overdueProjects > 0 ? C.red : C.green, onClick: () => setScreen("projects") },
                    ] : isLead ? [
                      { label: "Задач в отделе", value: baseTasks.length, color: C.accent, onClick: () => setScreen("tasks") },
                      { label: "В работе", value: baseTasks.filter(t => t.status === "inprogress").length, color: C.blue, onClick: () => setScreen("tasks") },
                      { label: "На проверке", value: baseTasks.filter(t => t.status === "review_lead" || t.status === "review_gip").length, color: C.purple, onClick: () => setScreen("tasks") },
                      { label: "Завершено", value: baseTasks.filter(t => t.status === "done").length, color: C.green, onClick: () => setScreen("tasks") },
                    ] : [
                      { label: "Мои задачи", value: baseTasks.length, color: C.accent, onClick: () => setScreen("tasks") },
                      { label: "В работе", value: baseTasks.filter(t => t.status === "inprogress").length, color: C.blue, onClick: () => setScreen("tasks") },
                      { label: "На проверке", value: baseTasks.filter(t => t.status === "review_lead" || t.status === "review_gip").length, color: C.purple, onClick: () => setScreen("tasks") },
                      { label: "Завершено", value: baseTasks.filter(t => t.status === "done").length, color: C.green, onClick: () => setScreen("tasks") },
                    ]).map(s => (
                      <div key={s.label} className="stat-card" onClick={s.onClick} style={{ cursor: 'pointer' }} onMouseEnter={e => (e.currentTarget.style.boxShadow = '0 4px 16px rgba(0,0,0,0.15)')} onMouseLeave={e => (e.currentTarget.style.boxShadow = '')}>
          ...
          {screen === "project" && activeProject && (
            <div className="screen-fade">
              ...
              {/* Tabs */}
              <div className="tab-strip" style={{ flexShrink: 0, overflowX: 'auto', scrollbarWidth: 'thin' }}>
                        ...
                        <div key={doc.id} style={{ display: 'grid', gridTemplateColumns: '28px 1fr 120px 100px 130px', gap: 0, padding: '10px 16px', alignItems: 'center', background: i % 2 === 0 ? C.surface : C.surface2, borderBottom: `1px solid ${C.border}` }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ fontSize: 11, color: C.textMuted }}>
                              {doc.status === 'ready' ? '✅ Готов' : doc.status === 'processing' ? '⚙️ Обрабатывается...' : doc.status === 'error' ? '❌ Формат не поддерживается' : '🕐 В очереди'}
                            </span>
                            {(isGip || isAdmin) && (
