...
export const globalSearch = async (query: string, token?: string) => {
  ...
  const safeGet = async (path: string) => {
    try {
      ...
      console.warn('[globalSearch] query failed:', path, e);
      return [];
    }
  };

  const [projects, tasks, drawings, reviews] = await Promise.all([
    safeGet(`projects?select=id,name,code,status&or=(name.${q},code.${q})&order=updated_at.desc.nullslast&limit=10`),
    safeGet(`tasks?select=id,project_id,name,dept,status,priority&or=(name.${q})&order=id.desc&limit=10`),
    safeGet(`drawings?select=id,project_id,code,title,discipline,status,revision&or=(title.${q},code.${q})&order=updated_at.desc.nullslast&limit=10`),
    safeGet(`reviews?select=id,project_id,drawing_id,title,status,severity&or=(title.${q})&order=id.desc&limit=10`),
  ]);
