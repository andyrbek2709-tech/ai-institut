                ...
                return (
                  <div key={task.id} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 5 }}>
                    ...
                    <div style={{ flex: 1, position: 'relative', height: 22, background: C.surface2, borderRadius: 4 }}>
                      ...
                      <div style={{ position: 'absolute', top: 3, bottom: 3, left: `${barL}%`, width: `${barW}%`, background: color, borderRadius: 3, opacity: 0.85, minWidth: 8 }} title={`${task.name} · ${sLabels[task.status]} · ${task.deadline || '—'}`} />
                    </div>
