...
### v7.3 — PDF-экспорт протоколов и трансмитталов
- ✅ **Экспорт протокола совещания в PDF**: кнопка 🖨 на каждой карточке протокола → генерирует A4 HTML с повесткой, решениями, строками для подписей → открывает диалог печати
- ✅ **Экспорт трансмиттала в PDF**: кнопка 🖨 на каждом трансмиттале → генерирует A4 Landscape HTML с таблицей чертежей (код, название, дисциплина, ревизия, примечание), строки Выдал/Принял
- ✅ **Нулевые зависимости**: HTML-to-print подход (Blob URL + window.print()), Cyrillic через OS-шрифты
- ✅ Файлы: `enghub-main/src/App.tsx` (функции `exportMeetingPdf`, `exportTransmittalPdf`), `enghub-main/src/components/TransmittalsTab.tsx` (prop `onExportPdf`)
...
#### [2026-04-01] v7.3 — PDF-экспорт протоколов и трансмитталов
- Step: Реализован HTML-to-print экспорт для протоколов совещаний (`exportMeetingPdf`) и трансмитталов (`exportTransmittalPdf`). Кнопки 🖨 добавлены на каждую карточку. Нулевые новые зависимости.
- Files: `enghub-main/src/App.tsx`, `enghub-main/src/components/TransmittalsTab.tsx`
- Validation: `npm run build` — успешно, +1.73 kB gzip.
- Next: Telegram-уведомления о дедлайнах / Глобальный поиск / GGE-модуль / Декомпозиция App.tsx.
